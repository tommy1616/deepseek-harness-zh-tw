...
`dsh-client-locale` 为 web GUI 提供本地化：用户在“设置 → 常规”中从已注册语言中选择，UI 文案会立即切换。本包内置 `zh`、`en` 与台湾繁体中文 `zh-TW`；外部 client 插件可以增加语言及其命名空间字典。`zh-TW` 会根据每个功能的 `zh` 字典自动生成，并套用台湾用语，因此现有插件会自动获得新语言；词汇规范也会统一使用“ 一般設定”、“引導傳送”、“檔案”、“資料夾”、“支援”、“載入”、“建置”、“工作流程”、“自訂”与“工作階段”等台湾产品用语。在 loopback 页面上，该选择以 `locale.preference` 存储在 `$DSH_HOME/settings.yaml` 中；非 loopback 页面即使由 Connection 认证所有 API 方法，也只在进程内保留选择。全新浏览器会先临时使用 `navigator` 请求的第一个已注册语言，直到允许读取的 Host 偏好到达并实时替换。插件作者使用内置字典形式时会获得完整类型检查，并通过框架 `t` 席位翻译；经 slot 渲染的文案会随语言切换即时更新。
...
用已合并进 `LocaleNamespaceMap` 的命名空间调用 `ctx.locale.register(ns, { zh, en })`；编译器会对照该命名空间的类型化键并集检查每个键，并要求基础 zh/en 词典齐全。运行时会自动根据 zh 注册对应的 `zh-TW` 词典。消费方通过 `ctx.locale.bind(ns)` 或框架注入的 `t` 席位翻译。UI 已挂载后再注册的词典无需重新挂载即可生效。
...
带类型的对象形式要求基础 `zh` 与 `en` locale 有完整字典；注册 zh 后会一并生成 `zh-TW`。逐 locale 形式允许语言包独立注册每个命名空间。逐键查找会先在请求命名空间中沿生效语言声明的 fallback 链查找，再在 `common` 中重复该链，最后显示键本身。绑定的翻译函数按命名空间保持稳定身份，因此可以挂在 inject 表面上而不破坏 memoization。
...
| [`src/locales/`](src/locales/) | 已发布的 `zh`／`en` 词典与 `zh-TW` 转换词汇 |
